...
def save_book(book:book.Book)->Book:
    ...
    try:
        ...
        db_session.rollback()
        db_book = db_session.query(Book).filter(Book.zip_name == book.zip_file.zip_name and Book.book_name == book.book_name).first()
        logging.error(e)
...
def save_book_word(book:Book, word: str):
    ...
    try:
        ...
        db_session.rollback()
        db_session.query(Word).filter(Word.word == word).update({Word.cnt : Word.cnt + 1})
